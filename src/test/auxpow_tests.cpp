...
public:
  ...
  explicit inline CAuxPowForTest (CTransaction&& txIn)
    : CAuxPow (std::move (txIn))
  {}

  using CAuxPow::getCoinbaseTransaction;
  using CAuxPow::getMerkleBranch;
  using CAuxPow::getChainMerkleBranch;
  using CAuxPow::getChainIndex;

  // Make CheckMerkleBranch public for testing
  static uint256 CheckMerkleBranch_test(uint256 hash,
                                        const std::vector<uint256>& vMerkleBranch,
                                        int nIndex) {
    return CAuxPow::CheckMerkleBranch(hash, vMerkleBranch, nIndex);
  }

  // Expose internal members for testing - now we can access them directly as friend
  CTransaction& coinbaseTx_test() { return coinbaseTx; }
  std::vector<uint256>& vMerkleBranch_test() { return vMerkleBranch; }
  std::vector<uint256>& vChainMerkleBranch_test() { return vChainMerkleBranch; }
  int& nChainIndex_test() { return nChainIndex; }
  CPureBlockHeader& parentBlock_test() { return parentBlock; }
};
...
CAuxpowBuilder::buildAuxpowChain (const uint256& hashAux, unsigned h, int index)
{
  ...
  const uint256 hash
    = CAuxPowForTest::CheckMerkleBranch_test (hashAux, auxpowChainMerkleBranch,
                                              index);

  std::vector<unsigned char> res(hash.begin(), hash.end());
...
CAuxpowBuilder::get (const CTransaction& tx) const
{
  ...
  CAuxPowForTest res(std::move(txCopy));
  
  // Build merkle branch for coinbase
  res.vMerkleBranch_test().clear();
  // For a single transaction, merkle branch is empty
  
  res.vChainMerkleBranch_test() = auxpowChainMerkleBranch;
  res.nChainIndex_test() = auxpowChainIndex;
  res.parentBlock_test() = parentBlock;
...
BOOST_FIXTURE_TEST_CASE (check_auxpow, TestingSetup)
{
...
BOOST_FIXTURE_TEST_CASE (auxpow_pow, TestingSetup)
{
